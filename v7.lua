...
local isAutoEnabled = false -- UIで切り替えるための変数
        ...
        obj.Triggered:Connect(function()
            ...
            if myBaseFolder and not obj:IsDescendantOf(myBaseFolder) then
                ...
                if target and root then
                    ...
                    -- 2. 「おーと」がオンなら、帰還直後にキャラTPを実行
                    if isAutoEnabled then
                        task.wait(0.1) -- 帰還判定のための極小の待ち
                        tpToEnemyAnimals()
                    end
                end
            end
...
-- ボタン配置（既存のUIコードの下に追加してください）
local autoBtn = Instance.new("TextButton", frame)
autoBtn.Name = "AutoLoop"
autoBtn.Size = UDim2.new(1, -20, 0, 35)
autoBtn.Position = UDim2.new(0, 10, 0, 195)
autoBtn.Text = "おーと:オフ"
applyModernStyle(autoBtn, Color3.fromRGB(127, 140, 141))
